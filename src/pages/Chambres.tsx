    ...
    <div>
              ...
              <motion.div key={room.id} variants={listItem} layout exit="exit">
                <GradientCard className="p-5 h-full flex flex-col">
                  ...
                  <div className="mt-4 flex items-center gap-1.5 border-t border-slate-200 pt-3">
                    ...
                    {can(perms, 'chambres', 'edit') && status === 'maintenance' && (
                      <button onClick={() => { endRoomMaintenance(room.id); toast.success(t('toast.updated')); }} title={t('rooms.endMaintenance')} className="grid h-9 w-9 place-items-center rounded-lg text-emerald-600 hover:bg-emerald-500/10 transition-colors">
                        <Check size={16} />
                      </button>
                    )}
      ...
      {formOpen && (
        <RoomFormModal
          ...
          onSave={(payload) => {
            if (formRoom) { updateRoom(formRoom.id, payload); toast.success(t('toast.updated')); }
            else { addRoom(payload); toast.success(t('toast.created')); }
            setFormOpen(false);
          }}
        />
      )}
      ...
      <ManageListModal
        ...
        items={floors}
        onAdd={(name) => { addFloor(name); toast.success(t('toast.created')); }}
        onDelete={(id) => { deleteFloor(id); toast.success(t('toast.deleted')); }}
      ...
      <ManageListModal
        ...
        items={categories}
        onAdd={(name) => { addCategory(name); toast.success(t('toast.created')); }}
        onDelete={(id) => { deleteCategory(id); toast.success(t('toast.deleted')); }}
      ...
      <MaintenanceModal
        ...
        onConfirm={(note) => {
          if (maintRoom) { setRoomMaintenance(maintRoom.id, note); toast.success(t('toast.updated')); }
          setMaintRoom(null);
        }}
      />
      ...
      <ConfirmDialog
        ...
        onConfirm={() => { if (toDelete) { deleteRoom(toDelete.id); toast.success(t('toast.deleted')); } }}
