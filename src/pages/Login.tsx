...
export default function Login() {
  ...
  // If no admin account exists yet, go straight to signup
  const hasAccount = accounts.length > 0;
  const [mode, setMode] = useState<'login' | 'signup'>(hasAccount ? 'login' : 'signup');
  ...
  const handleSignup = (e: React.FormEvent) => {
    ...
    const res = signup({ firstName, lastName, email, username, password: pw });
    if (res.ok) {
      toast.success(t('login.welcome'));
      navigate('/app/dashboard');
    } else {
            ...
            <AnimatePresence mode="wait">
              {mode === 'login' ? (
                ...
                >
                  ...
                  {/* Only shown before any account is created */}
                  {!hasAccount && (
                ...
                >
                  ...
                  {/* Back to login only relevant when an account already exists */}
                  {hasAccount && (
