...
export default function Reservations() {
  ...
  const handleDeleteOne = () => {
    if (!deleteOne) return;
    deleteReservation(deleteOne.id);
    toast.success(t('toast.deleted'));
  };

  const handleDeleteBulk = () => {
    const count = selectedIds.size;
    selectedIds.forEach((id) => deleteReservation(id));
  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        {r.notes && (
          <div className="rounded-xl bg-slate-100/70 border border-slate-200 p-4">
            <p className="text-[11px] font-bold text-ink-muted uppercase mb-2">{t('res.notes')}</p>
...
function PaymentModal({ reservation, onClose }: { reservation: Reservation | null; onClose: () => void }) {
  ...
  const save = () => {
    ...
    addPayment(r.id, Math.min(payNum, remaining), 'Paiement dette');
