...
export default function Expenses() {
  ...
  return (
    <div>
      ...
      <ConfirmDialog open={!!delExp} onClose={() => setDelExp(null)} onConfirm={() => { if (delExp) { deleteExpense(delExp.id); toast.success(t('toast.deleted')); } }} message={delExp ? `${t('common.deleteMsg')} (${delExp.name})` : ''} />
      <ConfirmDialog open={!!delMaint} onClose={() => setDelMaint(null)} onConfirm={() => { if (delMaint) { deleteMaintenance(delMaint.id); toast.success(t('toast.deleted')); } }} message={delMaint ? `${t('common.deleteMsg')} (${delMaint.name})` : ''} />
    </div>
  );
}
...
function ExpenseForm({ expense, onClose }: { expense: Expense | null; onClose: () => void }) {
  ...
  const save = () => {
    ...
    if (expense) { updateExpense(expense.id, payload); toast.success(t('toast.updated')); }
    else { addExpense(payload); toast.success(t('toast.created')); }
    onClose();
  };
...
function MaintenanceForm({ onClose }: { onClose: () => void }) {
  ...
  const save = () => {
    ...
    addMaintenance({ roomId, name: name.trim(), cost: Number(cost), date, description });
    ...
    <Modal open={open} onClose={onClose} title={t('common.category')} size="sm">
      <div className="space-y-3">
        <div className="flex gap-2">
          ...
          <GradientButton icon={<Plus size={17} />} onClick={() => { if (value.trim()) { addCategory(value.trim()); setValue(''); toast.success(t('toast.created')); } }}>{t('common.add')}</GradientButton>
        </div>
        <div className="space-y-2 max-h-72 overflow-y-auto">
          {categories.map((c) => (
            <div key={c.id} className="flex items-center justify-between rounded-xl bg-slate-100/70 border border-slate-200 px-4 py-2.5">
              ...
              <button onClick={() => { deleteCategory(c.id); toast.success(t('toast.deleted')); }} className="grid h-8 w-8 place-items-center rounded-lg text-ink-secondary hover:text-rose-600 hover:bg-rose-500/10 transition-colors"><Trash2 size={15} /></button>
            </div>
          ))}
        </div>
      </div>
